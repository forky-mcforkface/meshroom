    ...
    headerBar: RowLayout {
        Label {
            text: {
                if (node.isSubmittedOrRunning()) {
                ...
                } else if (node.isFinishedOrRunning()) {
                    ...
                    return "Ran for: " + getTimeStr(node.elapsedTime)
                }
            }
            padding: 2
            font.italic: true
            ...
            ToolTip.text: {
                if (node.isFinishedOrRunning() || (node.isSubmittedOrRunning() && node.elapsedTime > 0)) {
                    var longestChunkTime = getLongestChunkTime(node.chunks)
                    if (longestChunkTime > 0)
                        return "(longest chunk: " + getTimeStr(longestChunkTime) + ")"
                    else
                        return ""
                } else {
                    return ""
